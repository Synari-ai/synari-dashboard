...
import { DarkModeSidebar } from "@/app/components/dark-mode-sidebar"
import { DarkModeWalletConnect } from "@/app/components/dark-mode-wallet-connect"
...
export function DarkModeConsole() {
  ...
  if (!isConnected) {
    return (
      <div className="flex h-screen">
        <DarkModeSidebar />
        <main className="flex-1 bg-[#0F1115] p-8">
          <div className="flex justify-end mb-6">
            <DarkModeWalletConnect />
          </div>
          <div className="flex items-center justify-center h-[calc(100vh-150px)]">
            <div className="bg-[#1A1C23] p-8 rounded-lg shadow-md max-w-md w-full text-center border border-[#2A2D32]">
              ...
              <button
                onClick={() => router.push("/")}
                className="bg-[#2A2D32] text-white px-4 py-2 rounded-md hover:bg-[#3E4249] transition-colors"
              >
                Back to Dashboard
              </button>
            </div>
  ...
  return (
    <div className="flex h-screen">
      <DarkModeSidebar />
      <main className="flex-1 bg-[#0F1115] overflow-hidden">
        <div className="p-6">
          ...
          <div className="flex gap-4 h-[calc(100vh-180px)]">
            {/* Main Console Area */}
            <div className="flex-1 flex flex-col bg-[#0A0B0E] rounded-lg shadow-lg overflow-hidden border border-[#2A2D32]">
              {/* Console Controls */}
              <div className="bg-[#141517] px-4 py-2 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button onClick={() => setIsPaused(!isPaused)} className="p-1 rounded hover:bg-[#2A2D32]">
                          {isPaused ? (
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button onClick={clearLogs} className="p-1 rounded hover:bg-[#2A2D32]">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button onClick={downloadLogs} className="p-1 rounded hover:bg-[#2A2D32]">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className={`p-1 rounded hover:bg-[#2A2D32] ${showTimestamps ? "text-cyan-400" : "text-white"}`}
                        >
                          <Clock size={16} />
                        </button>
                      </TooltipTrigger>
                  ...
                  <Separator orientation="vertical" className="h-4 bg-[#2A2D32]" />
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 rounded hover:bg-[#2A2D32]"
                        >
              ...
              {/* Command Input */}
              <form onSubmit={handleCommandSubmit} className="bg-[#141517] p-2 flex gap-2">
                ...
                <input
                  ...
                  className="flex-1 bg-[#1A1C23] text-white font-mono p-2 rounded border border-[#2A2D32] focus:outline-none focus:border-cyan-500"
                />
            ...
            {/* Side Panel */}
            {showSidePanel && (
              <div className="w-64 bg-[#1A1C23] rounded-lg shadow-lg overflow-hidden flex flex-col border border-[#2A2D32]">
                <div className="bg-[#141517] px-4 py-3 font-medium flex justify-between items-center text-white">
                ...
                <div className="flex-1 overflow-auto">
                  {agents.map((agent) => (
                    <div
                      key={agent.id}
                      className="px-4 py-3 border-b border-[#2A2D32] flex items-center hover:bg-[#141517] cursor-pointer"
                ...
                <div className="bg-[#141517] px-4 py-2 text-xs text-gray-400">
                  {selectedAgents.length > 0
